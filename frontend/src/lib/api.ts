...
export async function fetchGlobals(): Promise<Globals> {
  ...
  try {
    ...
    return {
      ...
      aboutPageTitle: row.about_page_title ?? undefined,
      productionHeading: row.production_heading ?? undefined,
      careersHeading: row.careers_heading ?? undefined,
      careersEmailIntro: row.careers_email_intro ?? undefined,
      innLabel: row.inn_label ?? undefined,
      social: row.social ?? {},
